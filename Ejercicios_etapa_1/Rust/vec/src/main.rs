...
fn manipula_array<T: Clone>(primer_elemento: T, segundo_elemento: T, tercer_elemento: T) -> T {
  ...
  let mut vec_nuevo: Vec<T> = Vec::new();

  vec_nuevo.push(primer_elemento);
  vec_nuevo.push(segundo_elemento);
  vec_nuevo.push(tercer_elemento);
...
fn generar_rango(n: i32) -> Result<Vec<i32>, &'static str> {
    ...
    if n < 0 {
      return Err("Error n no puede ser negativo");
    }

    let mut rango: Vec<i32> = Vec::new();
    for i in 0..=n {
      rango.push(i);
    }
    Ok(rango)
...
fn organizar_por_tipo(arr: &Vec<Valor>) -> Organizado {
    ...
    for valor in arr {
      match valor {
        Valor::Numero(n) => numbers.push(*n),
        Valor::Texto(s) => strings.push(s.clone()),
      }
    }
    Organizado { numbers, strings }
}
    ...
    F: FnMut(T) -> U,
{
    ...
    let iterador = arreglo.into_iter();

    let iterador_transformado = iterador.map(fnc);

    let resultado: Vec<U> = iterador_transformado.collect();
...
fn usa_filter<T, F>(arreglo: Vec<T>, mut fnc: F) -> Vec<T>
where
    F: FnMut(&T) -> bool,
{
    ...
    let iterador_filtrado = iterador.filter(|elem| fnc(elem));

    let resultado = iterador_filtrado.collect();
...
fn usa_reduce<T, U, F>(arreglo: Vec<T>, mut fnc: F, valor_inicial: U) -> U
where
    F: FnMut(U, T) -> U,
{
    ...
    let resultado = iterador.fold(valor_inicial, |acumulador, elemento|{
      fnc(acumulador, elemento)
     });
